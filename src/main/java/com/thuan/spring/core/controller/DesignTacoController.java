...
import com.thuan.spring.core.domain.Ingredient;
import com.thuan.spring.core.domain.Ingredient.Type;
import com.thuan.spring.core.domain.Taco;
...
public class DesignTacoController {
	...
	public String showDesignForm(Model model) {
		...
		for (Type type : types) {
			...
			ingredients.stream().filter(in -> in.getType() == type).collect(Collectors.toList()));
			// @formatter:on
			model.addAttribute(type.toString().toLowerCase(),
					ingredients.stream().filter(in -> in.getType() == type).collect(Collectors.toList()));
